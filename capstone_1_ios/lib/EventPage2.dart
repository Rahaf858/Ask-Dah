...
import 'api_service.dart'; // خدمة API الجديدة
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> loadEvents() async {
    try {
      // استدعاء API للحصول على الأحداث
      final List<dynamic> jsonData = await ApiService.getEvents();
      
      // تحويل البيانات إلى التنسيق المتوقع من التطبيق
      List<Map<String, dynamic>> loadedEvents = [];
      for (var event in jsonData) {
        // تنسيق التاريخ من JSON (بافتراض أن start_date بتنسيق 'YYYY-MM-DD')
        String dateStr = event['start_date'] ?? '';
        DateTime? date;
        try {
          // تأكد من أن التاريخ بالتنسيق الصحيح YYYY-MM-DD
      ...
      // في حالة فشل تحميل البيانات من API، يمكن عرض حالة الخطأ للمستخدم هنا
      // أو تنفيذ استراتيجية احتياطية مثل عرض بيانات مخزنة محليًا
    }
  }
